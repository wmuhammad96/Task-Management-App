<template>
  <!-- <div >
        <ul class="grid justify-items-center pt-11">
            <li v-for="task in getTask" :key="task.id" class="flex ">
                <div>
                    {{ task.task }}
                </div>
                <div>
                    {{ task.status }}
                </div>
                <div>
                    {{ task.user }}
                </div>
            </li>
        </ul>
    </div> -->

  <div class="p-6">
    <!-- Header Row -->
    <div class="grid grid-cols-5 bg-gradient-to-r from-amber-950 to-amber-600 font-semibold border border-gray-300">
      <div class="px-4 py-2 border border-gray-300 text-amber-100 text-center">Project Name</div>
      <div class="px-4 py-2 border border-gray-300 text-amber-100 text-center">Status</div>
      <div class="px-4 py-2 border border-gray-300 text-amber-100 text-center">User</div>
      <div class="px-4 py-2 border border-gray-300 text-center text-amber-100">Edit</div>
      <div class="px-4 py-2 border border-gray-300 text-center text-amber-100">Delete</div>
    </div>
    <!-- Data Row -->
    <div v-for="task in getTask" :key="task.id" class="grid grid-cols-5 border border-gray-300">
      <div class="px-4 py-2 border border-gray-300 text-center">{{ task.task }}</div>
      <div class="px-4 py-2 border border-gray-300 text-center">{{ task.status }}</div>
      <div class="px-4 py-2 border border-gray-300 text-center">{{ task.user }}</div>
      <button @click="editTask"
        class="px-4 py-2 border border-gray-300 text-center text-blue-500 cursor-pointer">✏️</button @click="editTask">
      <button @click="deleteTask(task.id)"
        class="px-4 py-2 border border-gray-300 text-center text-red-500 cursor-pointer">🗑️</button>
    </div>
  </div>
</template>
<script setup>
import { TrashIcon } from "@vue-hero-icons/outline"
import { useStore } from 'vuex';
import { computed, ref, onMounted } from 'vue';
const store = useStore()
const editId = ref(null);
const editStatus = ref('');
const editRole = ref('');
// const getTask = ref(store.getters.getTask)
// let getTask = localStorage.getItem('res')

//   getTask = getTask ? JSON.parse(getTask) : []

const getTask = computed(() => store.getters.getTask)


onMounted(async () => {
  await store.dispatch("getTask");
})


const deleteTask = (id) => {
  store.dispatch('deleteTask', id)
}
const editTask = () => {

}


</script>