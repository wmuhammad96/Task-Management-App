<template>
  <div :class="drawerClasses"
    ...
    <div class="absolute inset-0 animate-gradient-slow bg-sky-600 opacity-50 pointer-events-none rounded-r-2xl blur-xl">
    </div>

    <ul class="relative flex flex-col gap-4 p-4">
      <li v-for="menu in drawerMenu" :key="menu.route" @click="navigate(menu.route)" :class="[
        ...
        <span
          class="absolute inset-0 rounded-xl bg-white/10 dark:bg-gray-100/10 blur-md opacity-0 transition-opacity duration-300 group-hover:opacity-80 pointer-events-none"></span>
      </li>
    </ul>
  </div>
</template>
